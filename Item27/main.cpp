...
/**
 * !尽量少做转型动作
 * C 风格的历史遗留问题：转型动作分两种：1. (T) expression  2. T(expression) 其中2实际上表示的是构造函数的隐式转换关系如：std::string str("Hello");
 * C++ 中提供了四种转型方案：
 ...
 * !需要注意的是：无论是 static_cast/ const_cast/ dynamic_cast/ reinterpret_cast 都会产生临时结果
 * 对于指针类型，转型动作会产生临时指针，而不是临时对象
 * 对于引用类型，转型动作不会产生任何东西
 * 对于值类型，转型动作会产生**临时对象**
 */
