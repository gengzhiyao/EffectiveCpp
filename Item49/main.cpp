...
/**
 * !了解 new_handler 的行为
 * Notion： 在 new 之前，必须先 set_new_handler，否则 new 会抛 std::bad_alloc
 * 异常 即便是类的
 * new_handler，在设置时也只能设置全局函数，不能设置类的成员函数（隐含this指针）或静态成员函数
 * 如果 new-handler 什么都不做，会无限循环，new_handler 要么释放内存，要么调用
 * std::abort() 退出程序，要么抛出异常
 */
...
{
public:
    static std::new_handler set_new_handler( std::new_handler handler ) // std::new_handler 的类型是
                                  // void(*)()，这里需要的是传入一个函数指针
    {
    ...
    {
        NewHandlerHolder holder( std::set_new_handler( m_currentHandler ) ); // std::set_new_handler
                                // 返回旧的new_handler，这里将其保存到 holder 中
...
std::new_handler Widget::m_currentHandler = nullptr; // 类的静态成员变量的定义，类内只是声明
...
int main( )
{
    ...
    // int* p = new int[922337203685477587];   // 32G 内存分配失败，调用
    // new_handler

    // Widget* p = new (std::nothrow) Widget;  // 常用形式，分配失败返回
    // nullptr，不抛异常 nothrow 保证的只是 operator new
    // 本身不抛，但构造函数内部仍然可能抛(构造函数内调用new再次分配内存)。所以
    // nothrow new 并不提供完全的异常安全保证，实用性有限。
    return 0;
};
